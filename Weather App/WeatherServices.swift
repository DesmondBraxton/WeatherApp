...
//  Weather App
//
//  Created by Desmond Braxton on 12/7/21.
//
...
public final class WeatherService: NSObject{
    ...
    private let API_KEY = "bf5427618f6e5fd6eed6cc68908f1500"
    ...
    private func makeDataRequest(forCoordinates coordinates: CLLocationCoordinate2D) {
        ...
        URLSession.shared.dataTask(with: url) {data, response, error in
            ...
            if let response = try? JSONDecoder().decode(APIResponse.self, from: data){ self.completionHandler?(Weather(response: response))}
            }.resume()
        }
